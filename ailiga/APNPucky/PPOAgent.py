...
class PPOAgent(nn.Module, Agent):
    ...
    def act(self, observation):
        return self.get_action_and_value(observation)[0].item()
...
def train_ppo(env):
    ...
    observation_size = env.observation_space(env.possible_agents[0])[
        "observation"
    ].shape
    print("observation_size", observation_size)

    """ LEARNER SETUP """
    agent = PPOAgent(num_actions=num_actions).to(device)
    ...
    for episode in range(total_episodes):
        ...
        with torch.no_grad():
            ...
            for step in range(0, max_cycles):
                ...
                print(next_obs, device)

                # rollover the observation
                obs = batchify_obs(next_obs["observation"], device)
